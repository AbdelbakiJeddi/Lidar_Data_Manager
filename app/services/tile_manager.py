...
class TileManager:
    ...
    async def process_dataset(self, dataset_id: str, tile_size: float = 500.0):
        ...
        try:
            ...
            origin_x = dataset.bbox.min_x
            origin_y = dataset.bbox.min_y

            logger.info(f"Splitting into {tile_size}m tiles with origin ({origin_x}, {origin_y})")
            raw_tile_paths = self.pdal.split_dataset_to_grid(
            ...
            for i, raw_tile in enumerate(raw_tile_paths):
                ...
                self.pdal.create_copc_tile(raw_tile, copc_file)
